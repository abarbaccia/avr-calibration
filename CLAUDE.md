...
- **DSP:** CamillaDSP via PipeWire → Focusrite Scarlett 18i20
  ...
  - Sub cal signal path: `pw-cat → avr_cal_sweep PW null sink → camilladsp_capture:input_3 (LFE feed → CamillaDSP → Scarlett → subs)`. **⚠️ The `input_3` link is LOAD-BEARING — it is the feed that drives the subs. Do NOT remove it; tearing it down silences the subs (coherence ~0.5, mic SNR ~0; verified 2026-06-10). `input_2` alone does NOT drive the subs.** The deconvolution reference is a SEPARATE tap: `avr_cal_sweep:monitor_FL → loopback_ref` (its own PW null-sink node), NOT a camilladsp_capture port. **⚠️ Second LOAD-BEARING link — the mic feed:** with `loopback_ref_pw_channels: 2` (the live config, R11 2-ch sample-locked capture) the MIC is `loopback_ref:monitor_FR`, fed by `UMIK:capture_FL → loopback_ref:playback_FR`. That link is REQUIRED (the mic source) — tearing it down silences the mic channel (`rec_1d=0` → "Sweep not detected"; cost a full session 2026-06-15). It is the OPPOSITE of the `UMIK→camilladsp_capture` feedback hazard (which must be removed). Note: with this link present, `check_system` "No Umik found" (PortAudio) is an EXPECTED red herring — `measure()` uses the loopback 2-ch capture, not PortAudio; gate on `measure()`, not the PortAudio mic check.
