...
# The AVR ingests filter coefficients as a stream of 531-byte binary frames.
# Layout: marker(1) + length(2) + seq(1) + last_seq(1) + "SET_COEFDT"(10)
#        + 0x00(1) + paramLen(2) + [first-pkt header (4 B): tc+sr+chan+0x00]
#        + N × IEEE-754 LE float32 + checksum(1).
# First packet carries 127 floats + the 4-byte header; subsequent packets
# carry up to 128 floats. Coefficients are NOT acked individually — caller
# is responsible for inter-packet pacing (typically CoefWaitTime.Init from
# GET_AVRINF, observed 0 ms on X3800H but a few ms is safer).

import struct as _struct

_MARKER: int = 0x54
_FIRST_PACKET_FLOAT_PAYLOAD: int = 127
_MID_PACKET_FLOAT_PAYLOAD: int = 128
_SET_COEFDT_BYTES: bytes = b"SET_COEFDT"


# Default per-channel target curves on X3800H float DType — both Flat (00)
# and Reference (01) banks are stored on the AVR; user toggles between them
# at runtime via AudyEqSet without re-uploading.
DEFAULT_TARGET_CURVES: tuple[str, ...] = ("00", "01")
DEFAULT_SAMPLE_RATES: tuple[str, ...] = ("00", "01", "02")  # 32k, 44.1k, 48k


def float_to_le32(x: float) -> bytes:
    """IEEE-754 little-endian float32 — the AVR's coefficient encoding."""
    return _struct.pack("<f", float(x))


def build_packet_config(total_floats: int) -> dict:
    """Compute the packet split for a given coefficient count.

    First packet carries 127 floats + a 4-byte header (tc+sr+channel+0x00).
    Subsequent packets carry 128 floats each. Last packet may be partial.

    Mirrors A1 Evo's ``buildPacketConfig`` (transfer.js) and OCA's
    ``build_packet_config`` (oca_transfer.py:622-672, MIT-licensed).
    """
    if total_floats <= 0:
        return {
            "totalFloats": 0,
            ...
            "firstPacketFloats": 0,
            "midPacketFloats": _MID_PACKET_FLOAT_PAYLOAD,
            "lastPacketFloats": 0,
        }
    if total_floats <= _FIRST_PACKET_FLOAT_PAYLOAD:
        packet_count = 1
        first = total_floats
        last = total_floats
    else:
        first = _FIRST_PACKET_FLOAT_PAYLOAD
        remaining = total_floats - first
        n_extra = (remaining + _MID_PACKET_FLOAT_PAYLOAD - 1) // _MID_PACKET_FLOAT_PAYLOAD
        packet_count = 1 + n_extra
        rem = remaining % _MID_PACKET_FLOAT_PAYLOAD
        last = rem if rem != 0 else _MID_PACKET_FLOAT_PAYLOAD
    return {
        "totalFloats": total_floats,
        "packetCount": packet_count,
        "lastSequenceNumField": format((packet_count - 1) & 0xFF, "02x"),
        "firstPacketFloats": first,
        "midPacketFloats": _MID_PACKET_FLOAT_PAYLOAD,
        "lastPacketFloats": last,
    }


def generate_coef_packets(
    coef_buffers: Sequence[bytes],
    config: dict,
    *,
    tc: str,
    sr: str,
    ...
    """Emit SET_COEFDT TCP frames for one channel/rate/curve stream.

    Args:
        coef_buffers: per-coefficient little-endian float32 bytes (use
            ``[float_to_le32(x) for x in taps]``).
        config: result of :func:`build_packet_config`.
        tc: target-curve hex code, ``"00"`` (Flat) or ``"01"`` (Reference).
        sr: sample-rate hex code (see SAMPLE_RATE_CODES — typically
            ``"00"``, ``"01"``, ``"02"`` for 32 / 44.1 / 48 kHz).
        channel_byte: from :func:`get_channel_byte`.

    Returns:
        List of fully-framed bytes ready for ``sock.sendall``. AVR sends
        no ACK on coefficient frames — caller is responsible for the
        inter-packet delay (typically ``CoefWaitTime.Init`` between
        channels).
    """
    if not coef_buffers:
        return []
    out: list[bytes] = []
    floats_processed = 0
    total_floats = len(coef_buffers)

    for packet_index in range(config["packetCount"]):
        is_first = packet_index == 0
        is_last = packet_index == config["packetCount"] - 1
        if is_first:
            n = config["firstPacketFloats"]
        elif is_last:
            n = config["lastPacketFloats"]
        else:
            n = config["midPacketFloats"]
        if n <= 0 or floats_processed >= total_floats:
            continue
        if floats_processed + n > total_floats:
            n = total_floats - floats_processed

        param_header = b""
        if is_first:
            channel_byte_hex = format(channel_byte, "02x")
            param_header = bytes.fromhex(tc + sr + channel_byte_hex + "00")

        payload = b"".join(coef_buffers[floats_processed: floats_processed + n])
        params_and_data = param_header + payload
        params_len = len(params_and_data)

        size_field = _struct.pack(">H", params_len)
        command_header = _SET_COEFDT_BYTES + b"\x00" + size_field

        # Total: marker(1)+length(2)+seq(1)+lastSeq(1)+header+data+checksum(1)
        total_packet_length = 1 + 2 + 1 + 1 + len(command_header) + params_len + 1
        last_seq_byte = bytes([int(config["lastSequenceNumField"], 16) & 0xFF])

        without_checksum = (
            bytes([_MARKER])
            + _struct.pack(">H", total_packet_length)
            + bytes([packet_index & 0xFF])
            + last_seq_byte
            + command_header
            + params_and_data
        )
        out.append(without_checksum + bytes([sum(without_checksum) & 0xFF]))
        floats_processed += n
    return out
...
def packets_for_channel(
    ...
    *,
    command_id: str,
    target_curves: Sequence[str] = DEFAULT_TARGET_CURVES,
    sample_rates: Sequence[str] = DEFAULT_SAMPLE_RATES,
    mult_eq_type: str = "XT32",
) -> list[tuple[str, str, list[bytes]]]:
    """Build the full SET_COEFDT packet stream for one channel.

    Returns a list of ``(target_curve, sample_rate, [packets])`` tuples
    in the order the AVR expects (outer loop = target curve, inner =
    sample rate, per A1 Evo's transfer.js and OCA's oca_transfer.py).

    The same coefficient set is sent at all sample rate × target curve
    combinations on X3800H. (A future extension could ship distinct
    filters per Flat vs Reference.)

    Length check: ``len(taps)`` must equal the AVR-format output length
    for ``command_id`` (1024 for speakers, 704 for subs under XT32).
    """
    cfg = filter_config_for(command_id)
    expected_n = cfg["output_length"]
    if len(taps) != expected_n:
        raise ValueError(
            f"taps length {len(taps)} != expected {expected_n} for "
            f"channel {command_id} ({mult_eq_type})"
        )
    cb = get_channel_byte(command_id, mult_eq_type=mult_eq_type)
    coef_buffers = [float_to_le32(x) for x in taps]
    pkt_cfg = build_packet_config(len(taps))
    streams: list[tuple[str, str, list[bytes]]] = []
    for tc in target_curves:
        for sr in sample_rates:
            pkts = generate_coef_packets(
                coef_buffers, pkt_cfg, tc=tc, sr=sr, channel_byte=cb,
            )
            streams.append((tc, sr, pkts))
    return streams
